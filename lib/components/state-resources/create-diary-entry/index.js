  ...
  async run (event, context) {
    ...
    if (entriesAtDate.length >= diary.slots.maxCapacity) {
      return context.sendTaskFailure({
        ...
        error: `Max. appointments of ${diary.slots.maxCapacity} already made on ${date}.`
      })
    }
    ...
    if (entriesAtDateTime.length >= diary.slots.maxConcurrency) {
      return context.sendTaskFailure({
        cause: 'createDiaryEntryFail',
        error: `Max. appointments of ${diary.slots.maxConcurrency} already made at ${event.startDateTime}.`
      })
    }

    if (diary.slots.endTime && diary.slots.startTime) {
      const startRule = moment(date + 'T' + diary.slots.startTime)
      const startIsSame = moment(event.startDateTime).isSame(startRule)
      const startIsBefore = moment(event.startDateTime).isBefore(startRule)

      if (!startIsSame && startIsBefore) {
        return context.sendTaskFailure({
          cause: 'createDiaryEntryFail',
          error: `The appointment must be after ${startRule.format('HH:mm')}.`
        })
      }
      ...
      if (!endIsSame && endIsAfter) {
        return context.sendTaskFailure({
          cause: 'createDiaryEntryFail',
          error: `The appointment must be before ${endRule.format('HH:mm')}.`
        })
      }
    }
    ...
    if (diary.slots.restrictions) {
      ...
      Object.keys(diary.slots.restrictions).forEach(restriction => {
        ...
        if (
          (isWithinRange(startRule, endRule, moment(event.startDateTime)) ||
